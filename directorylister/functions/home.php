					  ...
					  <tbody>
					    ...
					    <tr id = "id<?php echo $i; ?>">
						    ...
						    <td id = "info" class = "layui-hide-xs">
							    <!--如果是文件-->
							    <?php if($type == 'file'){ ?>
									<a href="javascript:;" title = "info" onclick = "filehash('<?php echo $showdir; ?>','<?php echo $fullpath; ?>')"><i class="fa fa-info-circle" aria-hidden="true"></i></a>
									<a href="javascript:;" onclick = "qrcode('<?php echo $showdir; ?>','<?php echo $url; ?>')" title = "QR code"><i class="fa fa-qrcode" aria-hidden="true"></i></a>
							    <?php } ?>
						    </td>
						    ...
						    <td class = "layui-hide-xs">
								<div class = "layui-hide-sm layui-show-md-block">
									<!--复制链接-->
									<?php if($fsize != '-'){ ?>
									<a href="javascript:;" class = "layui-btn layui-btn-xs layui-btn-normal" title = "Link" onclick = "copy('<?php echo $url ?>')"><i class="fa fa-copy"></i></a>
									<a download href="<?php echo $url ?>" class = "layui-btn layui-btn-xs layui-btn-normal" title = "Download"><i class="fa fa-download"></i></a>
									<?php } ?>
									<!--如果是音乐文件-->
									<?php if( $zdir->music($url) ) { ?>
									<a class = "layui-btn layui-btn-xs layui-btn-normal" title = "Play" href="javascript:;" onclick = "music('<?php echo $url ?>')"><i class="fa fa-play-circle"></i></a>
									<?php } ?>
									<!--音乐文件END-->
									<!--如果是markdown文件-->
									<?php if(($suffix == 'md') && ($suffix != null)){ ?>
									<a href="javascript:;" class = "layui-btn layui-btn-xs layui-btn-normal" onclick = "newmd('<?php echo $fullpath; ?>')" title = "Preview"><i class="fa fa-eye"></i></a> 
									<?php }else if( $zdir->video($url) ){ ?>
									<a class = "layui-btn layui-btn-xs layui-btn-normal" title = "Play" href="javascript:;" onclick = "video('<?php echo $url ?>')"><i class="fa fa-play-circle"></i></a>
									<!--文本查看器-->
									<?php }
									else if( $zdir->is_text($url) ){ ?>
									<a class = "layui-btn layui-btn-xs layui-btn-normal" title = "Preview" href="javascript:;" onclick = "viewtext('<?php echo $fullpath; ?>')"><i class="fa fa-eye"></i></a>
									<?php }else if( $zdir->office($url) ) { ?>
									<!--查看Office-->
									<a class = "layui-btn layui-btn-xs layui-btn-normal" href="javascript:;" title = "Preview" onclick = "office('<?php echo $url ?>')"><i class="fa fa-eye"></i></a>
									<?php } ?>
								</div>
						    </td>
							<!-- 操作例END -->
